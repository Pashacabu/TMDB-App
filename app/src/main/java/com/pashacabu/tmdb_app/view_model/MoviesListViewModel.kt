...
import com.pashacabu.tmdb_app.model.*
import com.pashacabu.tmdb_app.model.NetworkModule.Companion.api_key
import com.pashacabu.tmdb_app.model.data_classes.*
...
class MoviesListViewModel(private val database: Database, private val worker: WorkManager) :
    ...
    private val mutableScrollUp = MutableLiveData<Boolean>()
    ...
    val scrollUp: LiveData<Boolean> get() = mutableScrollUp
    ...
    private fun updateListBasedOnListOfFavourite(
        list: List<Movie?>?,
        listOfFavourite: List<Movie?>
    ): List<Movie?>? {
        list?.forEach { movieInCurrent ->
            checkIfInFavourite(movieInCurrent, listOfFavourite)
        }
        return list
    }
    ...
    fun updateLoadedListBasedOnFavourite(endpoint: String?) {
        if (!loadedList.isNullOrEmpty()) {
            viewModelScope.launch {
                ...
                val updatedList = updateListBasedOnListOfFavourite(list, listOfFavourite)
                updatedList?.forEach {
                    if (it != null) {
